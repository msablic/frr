	      ...
	      void *jarg)
{
	...
	while (1) {
		...
		for (h = (struct nlmsghdr*)buf; status >= (signed)sizeof(*h); ) {
			int err;
			int len = h->nlmsg_len;
			int l = len - sizeof(*h);
			...
			if (h->nlmsg_type == NLMSG_ERROR) {
				struct nlmsgerr *err = (struct nlmsgerr*)NLMSG_DATA(h);
				if (l < (signed)sizeof(struct nlmsgerr)) {
					fprintf(stderr, "ERROR truncated\n");
				} else {
					errno = -err->error;
					if (errno == 0) {
...
int addattr32(struct nlmsghdr *n, int maxlen, int type, __u32 data)
{
	...
	struct rtattr *rta;
	if ((signed)(NLMSG_ALIGN(n->nlmsg_len) + len) > maxlen) {
	      ...
	      int alen)
{
	...
	if ((signed)(NLMSG_ALIGN(n->nlmsg_len) + RTA_ALIGN(len)) > maxlen) {
...
{
	if ((signed)(NLMSG_ALIGN(n->nlmsg_len) + NLMSG_ALIGN(len)) > maxlen) {
...
int rta_addattr32(struct rtattr *rta, int maxlen, int type, __u32 data)
{
	...
	if ((signed)(RTA_ALIGN(rta->rta_len) + len) > maxlen) {
		  ...
		  const void *data, int alen)
{
	...
	if ((signed)(RTA_ALIGN(rta->rta_len) + RTA_ALIGN(len)) > maxlen) {
